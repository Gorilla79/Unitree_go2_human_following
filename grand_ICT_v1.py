...
class HostViewer(Node):
    def __init__(self):
        ...
        cv2.resizeWindow("Grand-Host", 1280, 900)
        cv2.namedWindow("Target-Image", cv2.WINDOW_NORMAL)
        cv2.resizeWindow("Target-Image", 1280, 720) 
    ...
    def on_meta(self, msg: String):
        ...
        try:
            ...
            if meta.get("ack") == "cmd":
               val = meta.get("value")
               print(f"[Host] ACK received from robot: cmd {val}")
               # 화면 상단에 잠깐 표시하도록 플래그 저장
               self._last_ack_text = f"ACK: cmd {val} received"
    ...
    def loop(self):
        # 좌표계 캔버스
        canvas = np.ones((900, 1280, 3), dtype=np.uint8) * 255
