...
class GrandICT(Node):
    def __init__(self):
        ...
        # 준비 동작: 균형기립 모드로
        self._call_cpp(["balance"])
        self._call_cpp(["speed","1"])  # 빠른 레벨
    ...
    def on_cmd(self, msg: String):
        ...
        if cmd == "0":
            ...
            # ACK 메타 전송
            m = String()
            m.data = json.dumps({"ack": "cmd", "value": "0", "ts": time.time()})
            self.pub_meta.publish(m)
            ...
            # ACK 메타 전송
            m = String()
            m.data = json.dumps({"ack": "cmd", "value": "1", "ts": time.time()})
            self.pub_meta.publish(m)
            ...
            # ACK 메타 전송
            m = String()
            m.data = json.dumps({"ack": "cmd", "value": "2", "ts": time.time()})
            self.pub_meta.publish(m)
    ...
    def _call_cpp(self, args):
        # args: ["move", vx, vy, vyaw] or ["stop"] ...
        cmd = ["sudo", "/home/unitree/unitree_sdk2-main/build/bin/go2_grand_ICT", "eth0"] + [str(a) for a in args]
        try:
        ...
        if self.cmd_mode == 0:
            ...
            if self.target_id is None:
                ...
                if sel:
                    ...
                    (u, v, Zmm) = sel[0]["uvZ"]
                    Xmm, Ymm = sel[0]["XYmm"]
                    # yaw는 u-u0에 비례, 전/후진은 Ymm - 3000
                    err_y = (Ymm - self.mm_target_follow)
                    err_u = (u - u0)

                    # 움직임 규모에 따라 빠른/느린 계수 선택
                    big_forward = abs(err_y) > 400.0  # 40cm 이상
                    big_yaw     = abs(err_u) > 80     # 80px 이상

                    if big_forward or (big_forward and big_yaw):
                        kv, kyaw = self.k_v_fast, self.k_yaw_fast
                        mv, myaw = self.max_v_fast, self.max_vyaw_fast
                        vx  = np.clip(err_y * kv / 1000.0, -mv, mv)
                        vyaw= np.clip(-err_u * kyaw,      -myaw, myaw)
                        self._call_cpp(["move", vx, 0.0, vyaw])
                    else:
                        kv, kyaw = self.k_v_slow, self.k_yaw_slow
                        mv, myaw = self.max_v_slow, self.max_vyaw_slow
                        vx  = np.clip(err_y * kv / 1000.0, -mv, mv)
                        vyaw= np.clip(-err_u * kyaw,      -myaw, myaw)
                        self._call_cpp(["move", vx, 0.0, vyaw])

                    # 최근 타겟 포즈 기록(approach 사용)
                    self.last_target_pose = (u, v, Zmm)
                else:
                    self.lost_frames += 1
                    if self.lost_frames <= self.lost_stop_delay:
                        self._move_slow(0.0, 0.0, 0.0)
                    else:
                        self._call_cpp(["stand"])

        elif self.cmd_mode == 2:
            # Approach@1m + Hello (최근 프레임 좌표 기반)
            base = None
            if self.target_id is not None:
                sel = [p for p in persons if p["id"] == self.target_id]
            ...
            if base is None:
                ...
                (u, v, Zmm) = base
                # 1m 접근
                err_y = (Zmm - self.mm_target_approach)
                err_u = (u - u0)
                # 가까이 오면 정지 + Hello
                if abs(err_y) < 120.0 and abs(err_u) < 40:
                    self._call_cpp(["stop"])
                    time.sleep(0.3)
                    ...
                    # 접근시엔 너무 빠르지 않게(안전)
                    vx  = np.clip(err_y * self.k_v_slow / 1000.0, -self.max_v_slow, self.max_v_slow)
                    vyaw= np.clip(-err_u * self.k_yaw_slow,      -self.max_vyaw_slow, self.max_vyaw_slow)
                    self._call_cpp(["move", vx, 0.0, vyaw])
